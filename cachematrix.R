...
makeCacheMatrix <- function(x = matrix()) {# Input a matrix
        i <- matrix()                       # Initialize i as a blank matrix, which will be the inverse of x
        set <- function(y) {                # Set the value of the matrix
                x <<- y                     # Assign to x the value of input argument
                i <<- matrix()              # Reset the inverse matrix i as a blank matrix since x is newly assigned
        }
...
cacheSolve <- function(x, ...) {            # Input a makeCacheMatrix object as an argument
        i <- x$getinverse()                 # Assign to i the cached value, if there is not a cached value, the value of i will be NA
